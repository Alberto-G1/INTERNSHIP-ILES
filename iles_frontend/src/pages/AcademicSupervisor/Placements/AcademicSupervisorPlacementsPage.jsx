...
const AcademicSupervisorPlacementsPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { label: 'Active', value: String(active), helper: 'Students in field', accent: '#5B82A6' },
      { label: 'Upcoming', value: String(upcoming), helper: 'Approved starts', accent: '#F59E0B' },
      { label: 'Completed', value: String(completed), helper: 'Finished period', accent: '#4DB87A' },
    ];
  }, [placements]);
  ...
  return (
    <PageScaffold
      ...
      subtitle="Monitor approved student placements assigned to your academic supervision"
      stats={stats}
    >
      <Stack spacing={1.2}>
        {loading ? (
          ...
          placements.map((p) => (
            <Box key={p.id} sx={{ border: '1px solid var(--gray-200)', borderRadius: '12px', p: 1.4 }}>
              ...
              <Stack direction="row" spacing={0.8} sx={{ mt: 0.8, alignItems: 'center' }}>
                <Chip size="small" label={p.current_lifecycle_status.toUpperCase()} sx={{ bgcolor: 'var(--blue-100)', color: 'var(--blue-700)' }} />
