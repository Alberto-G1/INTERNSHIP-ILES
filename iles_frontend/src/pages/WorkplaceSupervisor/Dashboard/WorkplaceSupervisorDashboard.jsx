...
const CHART_COLORS = [T.a700, T.t700, T.i700, T.violet, '#118AB2'];
...
const WorkplaceSupervisorDashboard = () => {
  ...
  const performanceComparison = students.map((student) => ({
    name: student.name,
    score: Number(student.progress || 0),
  }));

  const workloadChart = [
    { name: 'Pending', value: Number(stats.pendingReviews || 0) },
    { name: 'Reviewed', value: Number(stats.logsReviewed || 0) },
  ].filter((item) => item.value > 0);

  const weeklySubmissionTrend = useMemo(() => {
    const buckets = {};
    logs.forEach((log) => {
      const week = `W${Number(log.week_number || 0)}`;
      if (!buckets[week]) buckets[week] = { week, submissions: 0, approved: 0 };
      buckets[week].submissions += 1;
      if (log.review_status === 'approved') buckets[week].approved += 1;
    });
  ...
  return (
    <Box sx={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}>
          <SectionCard title="Review Workload" subtitle="Pending vs reviewed logs" delay={0.28}>
            <Box sx={{ height: 220 }}>
              {workloadChart.length === 0 ? (
                <Typography sx={{ fontSize: '12.5px', color: T.tx3 }}>No workload data yet.</Typography>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={workloadChart} dataKey="value" nameKey="name" outerRadius={72} label>
                      {workloadChart.map((entry, index) => (
        ...
        <Grid item xs={12} md={6}>
          <SectionCard title="Student Performance Comparison" subtitle="Average log approval progress by student" delay={0.3}>
            <Box sx={{ height: 230 }}>
              {performanceComparison.length === 0 ? (
                <Typography sx={{ fontSize: '12.5px', color: T.tx3 }}>No student comparison data available.</Typography>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={performanceComparison.slice(0, 8)}>
                    ...
                    <Tooltip />
                    <Bar dataKey="score" fill={T.t700} radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </Box>
        ...
        <Grid item xs={12} md={6}>
          <SectionCard title="Weekly Submission Trend" subtitle="Incoming logs and approvals by week" delay={0.32}>
            <Box sx={{ height: 230 }}>
              {weeklySubmissionTrend.length === 0 ? (
                <Typography sx={{ fontSize: '12.5px', color: T.tx3 }}>No trend data is available yet.</Typography>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={weeklySubmissionTrend}>
                    <CartesianGrid strokeDasharray="3 3" stroke={T.border2} />
                    <XAxis dataKey="week" tick={{ fontSize: 11 }} />
                    <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="submissions" stroke={T.i700} strokeWidth={2.2} dot={false} />
